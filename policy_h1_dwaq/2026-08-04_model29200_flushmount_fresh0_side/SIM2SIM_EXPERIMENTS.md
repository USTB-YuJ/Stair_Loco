...
### 2026-07-31 — global-best verification `model_50100.pt`, direct 0.6 m/s

- Video:
  `policy_h1_dwaq/2026-07-31_model50100_best_global/sim2sim_model50100_best_global_vx06_nosing_20s.mp4`
- Log:
  `policy_h1_dwaq/2026-07-31_model50100_best_global/sim2sim_model50100_best_global_vx06_nosing_20s.log`
- Checkpoint: `model_50100.pt` from
  `TienKung-Lab/logs/h1_dwaq/2026-07-28_19-09-18_stair_nosing1cm_baseheight6_airtime08_from25800/`.
...
- Command: 0.6 m/s for 20 s with automatic reset after returning to the
  approach flat.
- Result: the same behavior repeated twice. The robot armed at x=1.468 m,
  h=1.154 m; reached h=1.624 m at 4 s and h=2.124 m at 6 s; returned to the
  approach flat and reset at 9.32 s. The second attempt reset at 18.64 s.
  This is the strongest recorded practical-speed stair-climb behavior across
  the full experiment set, although it still does not complete the staircase.
- Safety: 15 plots were saved. Ankle target excess was 2.4566/2.3185 rad
  (left/right), ankle torque excess 61.7298/67.6204, and the largest knee
  torque excess was 106.9523. The policy remains sim-only pending safety
  review.
...
- Exported policy archive:
  `policy_h1_dwaq/2026-08-01_model54000_phasefreeze01/policy.pt`
- Policy SHA-256: `dc88ce8ec097c3c51fab8c1392198230cf10e4f704b49ec5e376f42275d3e380`
- Scene: `scene_payload_horizontal_nosing.xml`, with 1 cm noses on both
  ascending and descending stairs.
...
### 2026-08-01 — `model_71700.pt` repeat verification, 0.6 m/s

- Policy archive:
  `policy_h1_dwaq/2026-08-01_model71700_footsep/policy.pt`
- Video:
  `policy_h1_dwaq/2026-08-01_model71700_footsep/sim2sim_footsep_model71700_vx06_nosing_20s.mp4`
- Log:
  `policy_h1_dwaq/2026-08-01_model71700_footsep/sim2sim_footsep_model71700_vx06_nosing_20s.log`
- Checkpoint:
  `TienKung-Lab/logs/h1_dwaq/2026-07-30_21-03-35_phasefreeze02_footsep_fix_from55000/model_71700.pt`
- Scene and command: `scene_payload_horizontal_nosing.xml`, 1 cm noses on
  both stair directions, 0.6 m/s for 20 s, automatic reset on flat return.
- Result: both attempts reached about 1.534 m. The first reset at 11.72 s;
  the second repeated the same climb and was still at h=1.512 m at 19.7 s.
- Safety: 7 joint plots. Left/right ankle target excess 2.0417/1.6695 rad;
  ankle torque excess 41.9009/41.0059; left knee torque excess 20.1647.
- Video duration: 20.04 s (501 frames, 640x480 at 25 fps).
- Policy SHA-256:
  `bd4f1b8811f2412b35b9825f0812fd6af822eef8921d5a32913fe499dec78da8`.

### 2026-08-01 — `model_71700.pt`, straight-rear camera verification

- Video:
  `policy_h1_dwaq/2026-08-01_model71700_footsep_rear/sim2sim_model71700_vx06_rear20s.mp4`
- Log:
  `policy_h1_dwaq/2026-08-01_model71700_footsep_rear/sim2sim_model71700_vx06_rear20s.log`
- Same checkpoint, scene, command, auto-reset, and 20 s duration as the
  preceding verification. The camera was rotated to a straight rear view
  (`follow_yaw_offset=180°`, elevation `-15°`) to inspect foot yaw and lateral
  separation for possible in-toeing.
- Result: simulation trajectory and safety values match the preceding run;
  video contains 501 frames at 640x480 and 25 fps.

### 2026-08-01 — `model_70000.pt`, 0.6 m/s nose-stair sim2sim

- Policy archive:
  `policy_h1_dwaq/2026-08-01_model70000_footsep/policy.pt`
- Video:
  `policy_h1_dwaq/2026-08-01_model70000_footsep/sim2sim_footsep_model70000_vx06_nosing_20s.mp4`
- Log:
  `policy_h1_dwaq/2026-08-01_model70000_footsep/sim2sim_footsep_model70000_vx06_nosing_20s.log`
- Checkpoint:
  `TienKung-Lab/logs/h1_dwaq/2026-07-30_21-03-35_phasefreeze02_footsep_fix_from55000/model_70000.pt`
- Scene and command: `scene_payload_horizontal_nosing.xml`, 1 cm noses on
  both stair directions, 0.6 m/s for 20 s, automatic reset on flat return.
- Result: first attempt armed at t=2.50 s and reached h=1.617 m at 8 s,
  then reset at 10.80 s. The second attempt repeated the same trajectory and
  reached h=1.617 m at 18.8 s; it did not complete the staircase.
- Safety: 11 joint plots. Left/right ankle target excess 2.8834/1.4284 rad;
  ankle torque excess 72.3843/44.9382; right-knee torque excess 139.2575.
- Video duration: 20.04 s (501 frames, 640x480 at 25 fps).
- Policy SHA-256:
  `6486a268972bc6df8c781fc0859e95b7140f53df5365e3f14f7fbf8ce11b014b`.

### 2026-08-01 — `model_57000.pt`, 0.6 m/s nose-stair sim2sim

- Policy archive:
  `policy_h1_dwaq/2026-08-01_model57000_footsep/policy.pt`
- Video:
  `policy_h1_dwaq/2026-08-01_model57000_footsep/sim2sim_footsep_model57000_vx06_nosing_20s.mp4`
- Log:
  `policy_h1_dwaq/2026-08-01_model57000_footsep/sim2sim_footsep_model57000_vx06_nosing_20s.log`
- Checkpoint:
  `TienKung-Lab/logs/h1_dwaq/2026-07-30_21-03-35_phasefreeze02_footsep_fix_from55000/model_57000.pt`
- Scene and command: `scene_payload_horizontal_nosing.xml`, 1 cm noses on
  both stair directions, 0.6 m/s for 20 s, automatic reset on flat return.
- Result: each attempt reached only about h=1.248 m after the first nose and
  reset shortly afterward. Four repeat attempts occurred at 4.18, 8.36,
  12.54, and 16.72 s; the staircase was not continued.
- Safety: only 2 joint plots. Left/right ankle target excess 1.6248/1.1015
  rad; ankle torque excess 35.5497/24.2964.
- Video duration: 20.04 s (501 frames, 640x480 at 25 fps).
- Policy SHA-256:
  `4f262858dde70254ad206a7b9de725ff872f0005b4b2b8656b84697af87a4602`.

### 2026-08-01 — `model_57000.pt`, direct 0.8 m/s nose-stair sim2sim

- Video:
  `policy_h1_dwaq/2026-08-01_model57000_footsep_vx08/sim2sim_footsep_model57000_vx08_nosing_20s.mp4`
- Log:
  `policy_h1_dwaq/2026-08-01_model57000_footsep_vx08/sim2sim_footsep_model57000_vx08_nosing_20s.log`
- Same checkpoint and scene as the 0.6 m/s test; command was changed to
  0.8 m/s, with automatic reset on flat return.
- Result: four repeat attempts. Each reached about h=1.231 m and reset at
  5.18, 10.36, and 15.54 s; the fourth was still in the same pattern at the
  end of the 20 s video. It did not continue beyond the first nose.
- Safety: 5 joint plots. Left/right ankle target excess 1.6767/2.0902 rad;
  ankle torque excess 45.8417/42.8520.
- Video duration: 20.04 s (501 frames, 640x480 at 25 fps).
...
### 2026-08-01 — crossing-penalty continuation from `model_71700.pt`

- Status: running.
- Source checkpoint:
  `TienKung-Lab/logs/h1_dwaq/2026-07-30_21-03-35_phasefreeze02_footsep_fix_from55000/model_71700.pt`
- New run:
  `TienKung-Lab/logs/h1_dwaq/2026-08-01_18-50-22_phasefreeze03_crossing4_from71700`
- Console log:
  `TienKung-Lab/training_logs/train_phasefreeze03_crossing4_from71700.log`
- Change: increased `H1DwaqRewardCfg.feet_crossing` weight from `-2.0` to
  `-4.0`; `feet_too_near=-3.5` and `feet_y_distance=-0.6` were unchanged.
- Target: 4,100 additional iterations from 71,700 to the original 75,800
  iteration budget, 4,096 environments on GPU 0.
- The original config was backed up at:
  `rollback_backups/2026-08-01_before_crossing4_from71700/h1_dwaq_config_before_crossing4.py`.
- Initial health check: at iteration 71,739 the process was alive with no NaN
  or abnormal exit; mean reward had recovered to about 14.8 after the initial
  resume transient, and `Episode_Reward/feet_crossing` was about `-0.358`.

### 2026-08-03 — final checkpoint sim2sim after crossing-penalty continuation

- Training completed at `model_75799.pt` in
  `TienKung-Lab/logs/h1_dwaq/2026-08-01_18-50-22_phasefreeze03_crossing4_from71700`.
- Final training window: mean reward about 71.26, mean episode length about
  860.88, terrain level about 4.727, `feet_crossing` about -0.395, and
  `feet_y_distance` about -0.071. No NaN or abnormal training exit was seen.
- Sim2sim: 20 s on the 1 cm nose-stair scene at `vx=0.6 m/s`, with automatic
  reset on return to flat terrain and a straight-rear camera.
- Video:
  `policy_h1_dwaq/2026-08-03_model75799_crossing4_rear/sim2sim_model75799_crossing4_vx06_rear20s.mp4`
- Exported policy:
  `policy_h1_dwaq/2026-08-03_model75799_crossing4_rear/policy.pt`

### 2026-08-03 — low rear camera repeat

- Same checkpoint, scene, command (`vx=0.6 m/s`), duration (20 s), and reset
  rule as the preceding recording.
- Camera changed to a lower, wider side-rear view so both feet and the stair
  nose remain visible: distance 3.5 m, look-at height 0.45 m, elevation -8°,
  yaw offset 150°.
- Video:
  `policy_h1_dwaq/2026-08-03_model75799_crossing4_lowrear/sim2sim_model75799_crossing4_vx06_lowrear20s.mp4`

### 2026-08-03 — vertical payload and ordered-foot reward restart

- Source checkpoint:
  `TienKung-Lab/logs/h1_dwaq/2026-06-28_15-59-06_payload_resume_m18000_upright25_angvel008/model_25800.pt`
- New training run:
  `TienKung-Lab/logs/h1_dwaq/2026-08-03_13-50-07_vertical_payload_feetfix_from25800`
- Console log:
  `TienKung-Lab/training_logs/train_vertical_payload_feetfix_from25800.log`
- Root cause found in the lateral-spacing reward: the H1 default ankle-center
  separation is 0.40572 m, while `feet_y_distance` used a hard-coded 0.299 m
  target. This encouraged an approximately 10.7 cm narrower stance. In
  addition, the left/right body ordering was not explicitly preserved, and
  the signed crossing hinge did not activate until separation fell below
  0.04 m.
- Fixes: resolve left/right foot bodies with `preserve_order=True`; use a
  0.38 m spacing target with a 0.04 m tolerance; begin signed crossing
  shaping below 0.22 m; add a foot-yaw alignment penalty with a 0.10 rad
  deadband to address toe-in that the roll/pitch orientation term cannot see.
- Real-payload match: added vertical URDF/MJCF assets. The 0.405 m payload is
  aligned with the torso z-axis, centered at `(-0.18, 0, 0.25)` m, with the
  inertia axes rotated consistently. Both training and `sim2sim.sh` now use
  the vertical asset/scene.
- Training target: 50,000 additional iterations (25,800 to 75,800), 4,096
  environments on GPU 0. Startup health check reached iteration 25,810 at
  about 37k steps/s with finite rewards and all three new foot metrics logged.
- Rollback snapshot:
  `rollback_backups/2026-08-03_before_vertical_payload_feetfix_from25800`.

### 2026-08-03 — early vertical-payload evaluation at `model_29700.pt`

- Training state near evaluation: iteration 29,776, reward about 79–81,
  episode length about 825–849, and terrain level about 4.755. The weighted
  foot-yaw term improved from roughly -0.022 at startup to about -0.004.
- Sim2sim: 20 s at `vx=0.6 m/s`, vertical payload, 1 cm nose stair scene,
  low side-rear camera, and automatic reset after returning to flat terrain.
- Result: four repeats reached only the first stair (base height about 1.15 m)
  before stepping back and resetting. Foot yaw is less severe than at startup,
  but the stance remains visibly narrow/crossed in parts of the cycle; this
  checkpoint is not yet a satisfactory stair policy.
- Video:
  `policy_h1_dwaq/2026-08-03_model29700_vertical_feetfix_lowrear/sim2sim_model29700_vertical_vx06_lowrear20s.mp4`
- Exported policy:
  `policy_h1_dwaq/2026-08-03_model29700_vertical_feetfix_lowrear/policy.pt`
- Safety monitor: ankle target excess remained (left 1.264 rad, right 1.441
  rad), with torque excess about 54.5 and 73.3 respectively. This early
  checkpoint is not suitable for real-hardware deployment.

### 2026-08-03 — vertical-payload evaluation at `model_36500.pt`

- Training state near evaluation: iteration 36,555, reward about 85, episode
  length about 872, terrain level about 4.805, and roughly 37k steps/s.
- Sim2sim: `vx=0.6 m/s`, vertical payload, 1 cm nose stair scene. The robot
  reached base height 1.605 m before returning to flat and resetting at 6.52 s,
  a clear climbing improvement over `model_29700.pt`, which only reached about
  1.15 m.
- A diagnostic direct-rear camera was used (`yaw=180°`, elevation -15°,
  look-at height 0.75 m, distance 4.0 m). Recording was 10.08 s at 12 fps;
  this angle was later rejected because it did not match the requested first
  side view.
- Video:
  `policy_h1_dwaq/2026-08-03_model36500_vertical_feetfix_lowrear/sim2sim_model36500_vertical_vx06_rearfull10s.mp4`
- Safety monitor still reported ankle target/torque excess (left target 1.863
  rad and torque 68.5; right target 1.933 rad and torque 40.2), so this
  checkpoint is still simulation-only.

### 2026-08-03 — `model_36500.pt` restored original side view

- Same checkpoint, vertical-payload 1 cm nose stair scene, and `vx=0.6 m/s`
  as the preceding evaluation; only the camera was changed.
- Camera restored to the original/default side-rear view: `yaw=-45°`,
  elevation -28°, distance 3.0 m, and follow height 0.75 m. This is the
  user's preferred side angle for future evaluation videos.
- Recording: 10.084 s, 12 fps, 480x360. The robot reached base height about
  1.605 m, returned to flat, and automatically reset at 6.52 s.
- Video:
  `policy_h1_dwaq/2026-08-03_model36500_vertical_feetfix_side/sim2sim_model36500_vertical_vx06_first_side10s.mp4`
- Exported policy:
  `policy_h1_dwaq/2026-08-03_model36500_vertical_feetfix_side/policy.pt`

### 2026-08-04 — fresh-policy evaluation at `model_29200.pt`

- Checkpoint comes from a true random initialization (no checkpoint was loaded),
  with the corrected feet-spacing/crossing/yaw rewards and the vertical
  extinguisher payload.
- Payload nominal mount matches the latest hardware photo: its front face is
  2 mm behind the torso back surface, and the payload top is 1 cm above the
  torso back top. Training randomization is limited to realistic mount
  tolerances (x/z ±5 mm, y ±10 mm, rotations ±0.05 rad).
- Evaluation: 10.084 s, `vx=0.6 m/s`, 1 cm nose stair scene, and the original
  user-preferred side-rear camera (`yaw=-45°`, elevation -28°, distance 3 m).
- Training state at evaluation: iteration 29,246/75,800, mean reward about
  70–79, mean episode length about 800–884, terrain level about 4.63, with no
  NaN or memory error. This is an intermediate, simulation-only policy.
- Video:
  `policy_h1_dwaq/2026-08-04_model29200_flushmount_fresh0_side/sim2sim_model29200_flushmount_vx06_side10s.mp4`
