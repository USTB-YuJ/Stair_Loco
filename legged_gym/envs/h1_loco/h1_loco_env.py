...
class H1_Loco_Robot(LeggedRobot):
    ...
    def get_amp_observations(self):
        # keep the same convention as G1: use joint positions as AMP state
        return self.dof_pos

    def _get_noise_scale_vec(self, cfg):
        """Noise scaling for observation structure.
        ...
        noise_vec[
            9 + self.num_actions : 9 + 2 * self.num_actions
        ] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
        noise_vec[
            9 + 2 * self.num_actions : 9 + 3 * self.num_actions
        ] = 0.0
    ...
    def _init_buffers(self):
        ...
        self.last_feet_contact_force = torch.zeros(
            self.num_envs, 2, 3, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.last_last_feet_contact_force = torch.zeros(
            self.num_envs, 2, 3, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.feet_indicator_offset = torch.tensor(
            self.cfg.asset.feet_indicator_offset, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.feet_indicator_pos = torch.zeros(
            self.num_envs,
            len(self.feet_indices),
            *self.feet_indicator_offset.shape,
            dtype=torch.float,
            device=self.device,
            requires_grad=False,
        )
    ...
    def reset_idx(self, env_ids):
        ...
        self.last_last_feet_contact_force[env_ids] = 0.0
    ...
    def _reset_dofs(self, env_ids):
        # Keep consistent with G1: randomize around default pose.
    ...
    def post_physics_step(self):
        """Mirror the G1 loco post step so runners can consume terminal AMP states."""
        ...
        if self.viewer and self.enable_viewer_sync and self.debug_viz:
            if self.cfg.depth.use_camera and self.cfg.depth.warp_camera:
                ...
                cv2.waitKey(1)
                cv2.namedWindow("Warp Depth Image", cv2.WINDOW_NORMAL)
    ...
    def compute_feet_indicator_pos(self):
        ...
        quat_expanded = ankle_quat.unsqueeze(2).expand(-1, -1, num_dot, -1)
        ...
        self.feet_indicator_pos = rotated_points + self.feet_pos.unsqueeze(2)

    def check_termination(self):
        self.reset_buf = torch.any(
            torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 1000.0, dim=1
        )
        self.reset_buf |= torch.logical_or(torch.abs(self.rpy[:, 1]) > 1.0, torch.abs(self.rpy[:, 0]) > 0.8)
        self.reset_buf |= self._get_base_heights() < 0.45
    ...
    def compute_observations(self):
        ...
        if self.cfg.env.feet_info:
            self.privileged_obs_buf = torch.cat(
                (
                    self.privileged_obs_buf,
                    self.footpos_in_body_frame.reshape(self.num_envs, -1),
                    self.footvel_in_body_frame.reshape(self.num_envs, -1),
                ),
                dim=-1,
            )

        if self.cfg.env.foot_force_info:
        ...
        if self.cfg.env.priv_info:
            ...
            if self.cfg.domain_rand.randomize_com_pos:
                self.privileged_obs_buf = torch.cat(
                    (
                        self.privileged_obs_buf,
                        self.randomized_com_pos * self.obs_scales.com_pos,
                    ),
                    dim=-1,
                )
    ...
    def _reward_feet_slippage(self):
        return torch.sum(
            torch.norm(self.feet_vel, dim=-1) * (torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1) > 1.0),
            dim=1,
        )
    ...
    def _reward_feet_contact_momentum(self):
        ...
        return torch.sum(torch.abs(feet_contact_force * feet_vertical_vel), dim=-1)
    ...
    def _reward_feet_edge(self):
        foot_indicators_pos_xy = (
            (self.feet_indicator_pos[..., :2] + self.terrain.cfg.border_size) / self.cfg.terrain.horizontal_scale
        ).round().long()
