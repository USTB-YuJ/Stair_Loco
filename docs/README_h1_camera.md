...
> **独立部署仓库**：若使用本仓库根目录下的实机脚本（`deploy_real_h1_camera.py`、`configs/`、`weights/`），请以仓库根目录 [README.md](../README.md) 为准；下文「文件清单」中 MuJoCo 与原始 `deploy_camera` 路径仍描述训练侧母仓库布局，供对照。
...
| `deploy_camera/deploy_real/deploy_real_h1_camera.py`     | 实机部署脚本 (Unitree SDK + RealSense) — 母仓库路径 |
| `deploy_camera/deploy_real/configs/h1_camera.yaml`       | 实机部署配置 — 母仓库路径 |
| **本独立仓库** `deploy_real_h1_camera.py` / `configs/h1_camera.yaml` | 与母仓库逻辑等价，路径相对本仓库根目录，见根目录 `README.md` |
...
- 相机外参（`camera_parent_body = torso_link`，与 `h1_description/urdf/h1.urdf` 一致）：
  ...
  - `y_angle` ≈ 50.8° 俯仰（域随机化 `[48, 54]`）
  - `fovy_range = [58, 62]`°（对齐 D435i 深度 **垂直** FOV）
  - 图像分辨率 **48 × 64**（H×W，与 640×480 同 4:3，可直接 `resize` 无需裁剪）
...
mj_step ──┬─► PD: tau = kp(target_dof_pos − q) − kd · dq
          ...
          └─► 每 control_decimation (=10) 步触发一次：
                ...
                │     渲染 depth_cam → 48×64 → 归一化 → 推入 depth_buffer
...
- 头部/躯干按 URDF 安装一台 Intel RealSense（推荐 D435i），外参与训练
  `depth.position` / `y_angle` / `fovy_range` 一致（见上表与 `h1_camera.xml`）。
...
（若仍使用原始 `legged_gym` 母仓库中的 `deploy_camera`，则按其原路径 `cd` 到对应目录后运行同名脚本。）
...
raw uint16 (mm)  →  meters (×depth_scale)
                  →  无效值 (==0) 视为远端 (= far_clip)
                  →  resize 到 48×64 (INTER_AREA)，与训练 `depth.resized` 一致
                  →  clip 到 [0, 2] m
                  →  归一化到 [-0.5, 0.5]
```
...
| `depth.resized` | (48, 64) | RealSense / MuJoCo 输出均 resize 到 48×64 |
| `depth.fovy_range` | [58, 62]° | MuJoCo: `fovy="60"` 量级；匹配 D435i **VFOV** |
| `depth.position` | (0.10848, 0.01750, 0.69317) torso 系 | MuJoCo `<camera pos=...>`；实机机械对位 |
| `depth.y_angle` | [48, 54]°（≈50.8°） | MuJoCo: `euler="0 -0.886627 0"`；实机俯仰 |
...
- **深度图异常**
  ...
  - 若 RealSense 与训练 FOV 仍有偏差，优先机械对位；必要时再在 `_loop` 中
    加裁剪/缩放，但默认 **640×480 → 48×64** 已与训练纵横比一致。
- **JIT 加载报错**
